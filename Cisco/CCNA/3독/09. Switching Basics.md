...
If throughput is a big concern, you may want to buy a trunk NIC for the file server. Configuring this NIC is different from configuring a normal NIC on a file server. For each VLAN in which you want the file server to participate, you would create a virtual NIC, assign your VLAN identifier and layer 3 addressing to the virtual NIC for the spcific VLAN, and then associate it with the physical NIC.

#### IEEE 802.1Q

InterSwitch Link (ISL), a Cisco proprietary protocol, is being phased out in Cisco's products and being replaced with the IEEE 802.1Q trunking standard.

One of the advantages provided by the IEEE standard is that it allows trunks between different vendors' devices. Therefore, by using 802.1Q you should be able to implement a mutlivendor trunking solution without having to worry about whether or not a specific type of trunk connection is or is not supported.
...
One of the unique aspects of 802.1Q trunking is that you can have *both* tagged and untagged frames on a trunk connection. One restriction placed on an 802.1Q trunk configuration is that it must be the *same* on both sides.

With the 802.1Q tagging method, the original Ethernet frame is modified. A 4-byte *tag* field is *inserted* into the header of the original Ethernet frame, and the original frame's FCS (checksum) is recomputed on the basis of this change.
The first 2 bytes of the tag is the protocol identifier. The next 3 bits are used to prioritize the frame, which is defined in the IEEE 802.1p standard. The fourth bit indicates if this is an encapsulated Token Ring frame, and the last 12 bits are used for the VLAN identifier (number).

![[Pasted image 20251214171617.png]]

### Configuring Trunks

#### Dynamic Trunking Protocol

Cisco's proprietary *Dynamic Trunking Protocol (DTP)* is used to form and verify a trunk connection dynamically between two Cisco switches using a trunk conneciton.

It supports five trunking modes.

| DTP Mode     | Generate DTP Message | Default Time Tagging |
| ------------ | -------------------- | -------------------- |
| On or trunk  | Yes                  | Yes                  |
| Desirable    | Yes                  | No                   |
| Auto         | No                   | No                   |
...
| Off          | No                   | No                   |
* **On / trunk**
	* Interface will generate DTP messages and tag frames.
	* Interface always assumes the connection is trunk.
* **desirable**
	* The interface will generate DTP messages on the interface, but it will make the assumption that the other side is not trunk-capable and will wait for a DTP reply message from the remote side.
	* The interface starts as an access port connection. If the remote side sends a DTP message, and this mesasge indicates that trunking is compatible between the two switches, a trunk will be formed and the switch will start tagging frames on the interfaces.
	* If the other side does not support trunking, the interface will remain as an access port connection.
* **auto**
	* The interface listens for DTP messages from the remote side and leaves the interface as an access port connection.
	* If the interface receives a DTP message, and the message matches trunking capabilities on the interface, then the interface will change from an access port connection to a trunk connection and start tagging frames.
	* This is the default DTP mode for a Cisco switch interface that is trunk-capable.
* **no-negotiate**
	* The interface is set as a trunk connection and will automatically tag frames with VLAN information; however, the interface will not generate DTP messages: DTP is disabled.
	* Typically used when connecting trunk connections to non-Cisco devices that don't understand DTP.
* **off**
	* This interface is configured as an access port.
	* No DTP, no tagging.

**Forming Trunks**

| local switch | Remote switch       |
| ------------ | ------------------- |
...
```
switch(config-if)#switchport trunk encapulation dot1q
switch(config-if)#switchport mode trunk|dynamic desirable|
							dynamic auto|nonegotiate

### for 802.1Q trunks, the native VLAN is VLAN 1. You can change with this
# If two interconnected switches have a different native VLAN on the connecting trunks, then traffic from one VLAN on one switch will end up in a different VLAN on the remote switch.
switch(config-if)#switchport trunk native vlan VLAN_#
```

After you have configured your trunk connection, you can verify with `show interfaces TYPE switchport|trunk` command.
...
* The number of VLANs you can create is dependent on the switch model and IOS versions
* Some VLANs are preconfigured on every switch, including VLAN 1 and 1002-1005 (Token Ring and FDDI networks only). These VLANs cannot be deleted
* To add or delete VLANs, your switch must use enter VTP server or transparent mode
* VLAN names can be changed, but VLAN numbers can't; you must delete a VLAN and re-add it in order to renumber it.
* All interface, by default belong to VLAN 1.
...
Switch(config-vlna)#name VLAN_NAME
```
...
### specify the VLAN to be used for VoIP devices (optional)
Switch(config-if)#switchport voice vlan VLAN_#
```

You can use various `show` commands to review and verify your configuration.
The `show vlan` command displays the list of VLANs and which ports are assgined to them. Add the `brief` parameter to this command, and it will not display the details for each VLAN at the bottom of the display.
...
1. Check the status of yoru interface to determine whether it is a physical layer problem
...
If you are experiencing slow performance or intermittent connection problems, you should first check the statistics on the interface of your switch with the `show interfaces` command.

A few things can cause these problems. The most common is a mismatch in either the duplexing or the speed on a connection. Examine the settings on both sides of the connection. Also make sure that you are using the correct cabling type: stright for a DTE-to-DCE connection and crossover for a DTE-to-DTE or DCE-to-DCE connection. And make sure that the cable does not exceed the maximum legal limit. Also make sure that the connected IC is not experiencing a hardware problem or failure.

#### Local Connection Problems

If you are attemping to access to across the console port of a switch or router, and all you see is garbage in your terminal session, this could indicate an incorrect terminal setting. Usually the culprit is an incorrect baud rate.
Some devices allow you to perform an operating system upgrade via the console port, and an administrator might change it to the highest possible value but forgot to change back to 9600 bps. If you suspect this, keep on changing your baud rate until find the right speed.

If you are having problems accessing devices in the switched network, you can look at a few options. First, is the device you are trying to reach in the same VLAN? If so, make sure that you are using the correct IP addressing scheme in the VLAN and that the two devices trying to share information have their ports in the same VLAN. If the two devices are Cisco devices, you can use CDP to elicit some of this information. For instance the IP address, by using the `show cdp` command.

For VLAN information, use the `show` commands on your switches to check your VLAN configuration. Also check the VLAN configuration on each switch and make sure the VLANs are configured with the same parameters by using the `show vlan` command. If you see that a lot of port LEDs are amber, a deleted VLAN is probably the problem. Use the `show interface switchport` command to determine whether a deleted VLAN is the problem.

If you are using trunks between the switches, make sure that the trunks are configured correctly: `show itnerface trunk`. Make sure that the native VLAN number matches on both ends of the trunk: if they are mismatched, a trunk will not form.

Also check VTP (if you are using it) by executing the `show vtp` command. Make sure you have trunk connections between your switches, since VTP messages only traverse trunks. When using a server/client implementation, make sure that the domain name and, if using the password option, the password match among the VTP switches. Also, all switches must be running the same VTP version.

#### Inter-VLAN Connection Problems

If you are having problems reaching devices in other VLANs, make sure that, first, you can ping the default gateway (router) that is your exit point from the VLAN. A common misconfiguration on a user's PC is a misconfigured default gateway.
...
Also make sure that the router has a route to the destination VLAN (`show ip route`). If you do have a router to the destination, make sure the destination VLAN is configured correctly and the default gateway in that VLAN can reach the destination device.
...
LA-SW1(config)#interface g1/0/1
LA-SW1(config-if)#switchport trunk encapsulation dot1q
LA-SW1(config-if)#switchport mode trunk

### verify trunk
VAN-SW1#show interfaces trunk
```
...
* Layer 2 switches have three operation modes: store-and-forward, cut-through, and fragment-free.

### Basic Switch Configuration

* `ip default-gateway`
	* Assigns a router address the switch should use to access other subnets
